...
Esta análise foi conduzida sobre dados públicos simulados e agregados de pontualidade de voos comerciais (baseados no dataset 'Flight Delay and Cancellation' da FAA/Kaggle), adaptados para demonstrar metodologias de classificação preditiva e Explainable AI (XAI). As métricas de receita, economia e os benchmarks de aeronaves refletem estimativas e cenários hipotéticos de estudo de caso para validação de conceitos de negócio.
...
*   **Receita em Risco (Diária):** Monitoramento de receita sob risco estimada em cerca de **R$ 4.8M** (cenário simulado) em passagens em rotas com baixa confiabilidade.
*   **Redução de Custo (OPEX):** Potencial de redução estimada de **15% em gastos com reacomodação** (cenário simulado) e multas operacionais via realocação tática de frota.
...
*   **ROI Projetado (Simulação):** Aumento projetado de disponibilidade de frota de até **12% no trimestre** (cenário simulado).
...
*   **Impacto no NPS:** Redução projetada de **20 pontos na detratação** (cenário simulado) por atrasos em passageiros corporativos.
...
![Drivers de Riscos Operacionais - SHAP Summary](../../reports/figures/aviation_ops/shap_summary_aviation.png)
